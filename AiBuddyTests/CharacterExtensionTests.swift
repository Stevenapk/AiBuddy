...
class CharacterExtensionTests: XCTestCase {
    ...
    func testSortedMessages() {
        ...
        mockCharacter.addToMessages(NSSet(array: [message1, message2, message3]))
        
        let sortedMessages = mockCharacter.sortedMessages
    ...
    func testDeleteAllMessages() {
        ...
        mockCharacter.deleteAllMessages {
            XCTAssertEqual(mockCharacter.messages?.count, 0, "All messages should be deleted")
            XCTAssertEqual(mockCharacter.lastText, "", "lastText should be empty")
        }
    }
